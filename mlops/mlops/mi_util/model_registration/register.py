...
def latest_registered_model_details(registere_model_name, experiment_id):
    registered_prediction_model = mlflow.search_registered_models(
        filter_string=f"name='{registere_model_name}'"
    )
...
def register_model(
    ...
    reg_run_id, reg_rmse = latest_registered_model_details(
    ...
    if reg_rmse == None or reg_rmse > runs[0].data.metrics['val_rmse']:
        ...
        if reg_rmse == None:
            print(
                f"Registered First version of model with rmse:{runs[0].data.metrics['val_rmse']}"
            )
        else:
            print(
                f"Latest run id's {runs[0].info.run_id} performance improved by {reg_rmse - runs[0].data.metrics['val_rmse']} rmse. old rmse {reg_rmse}"
            )
        return True
    else:
        print(
            f"Model Performance doesn't improve, already registered run id's {reg_run_id}, rmse:{reg_rmse} performance is good till now."
        )
        return False
